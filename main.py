...
def send_message(message):
    if message.text.lower() == ('справка'):
     mess = (f'Приветствую вас, <b>{message.from_user.first_name}</b>, Вот все запросы на которые я знаю ответы, пришлите'
             f' мне нужный вам запрос так же как написано в списке ниже:'
             f'Расписание всех автобусов'
             f'Узнать ближайшее время'
             f'Номера телефонов')
    bot.reply_to(message, mess, parse_mode='html')

    if message.text.lower() == ('расписание всех автобусов'):
     mess = (f'Выберите нунжный вам маршрут из списка и отправьте мне цифру или название маршрута:'
                    f'1, 3, 4, 4а, 5, 6, 8, 9, 10, 13, 14, 17, 19, 20, 21, 22, 23 , 24, 25, 103, 104, 105, 118, 119, 122, '
                    f'Канск-Иланск, пригородные, междугородные, Иланск, Иланск-Красноярск, Красноярск-Восток')
    bot.reply_to(message, mess, parse_mode='html')
    if message.text.lower() == ('Узнать ближайшее время'):
     mess = (f'Здесь можно узнать ближайшее время автобуса на текущее время запроса, пока что я знаю мало маршрутов для '
             f'этого запроса, в скором времени список расширится. Выберите из списка нужный вам маршрут и напишите мне '
             f'так же как написано в кавычках с номером нужного вам маршрута "ближайший № маршрута"'
             f'№ маршрута:'
             f'20, 21, 22, 25')
    bot.reply_to(message, mess, parse_mode='html')
    if message.text.lower() == ('номера телефонов'):
     mess = (f'+73912635500- Министерство транспорта Красноярского края'
             f'+73916135400, 83916135170- Служба заказчика'
             f'kansk_us_jkx@mail.ru -сюда можно так же отправлять жалобы'
             f'ПАТП:'
             f'+73916124480- диспетчер'
             f'2-44-81- Директор Канского филиала АО "Краевое АТП"'
             f'Коммерческой диспетчерской:'
             f'+79135079724 -солнечный'
             f'Справочных автовокзалов:'
             f'Канск +73916132261'
             f'Нижний Ингаш +73917121883'
             f'Иланский +73917331235'
             f'Зеленогорск +73916928001'
             f'Абан +73916135660, +73916322529'
             f'Красноярск +73912201172')
    bot.reply_to(message, mess, parse_mode='html')
